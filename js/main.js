...
if (mobileNavClose) mobileNavClose.addEventListener('click', window.closeMobileNav);
mobileLinks.forEach(link => link.addEventListener('click', window.closeMobileNav));
  ...
  <div class="product-card reveal up reveal-d${(idx % 3) + 1}" data-id="${id}">
    ...
    <div class="product-body">
      ...
      <div class="product-footer">
        ...
        <button class="btn-order" onclick="openModal('${p.name}','${p.price}')">اطلبي الآن</button>
      </div>
    </div>
  </div>`;
}
...
window.openModal = (product, price) => {
  ...
  ['fName','fPhone','fCity','fAddress','fMeters'].forEach(id => {
    ...
    if (el) { el.value = id === 'fMeters' ? 1 : ''; el.classList.remove('error'); }
  });
  document.querySelectorAll('.field-err').forEach(e => e.classList.remove('show'));

  if (typeof L !== 'undefined') {
    if (!map) {
      map = L.map('map', { zoomControl: true }).setView([selectedLat, selectedLng], 16);
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: '© OpenStreetMap contributors'
      }).addTo(map);
      marker = L.marker([selectedLat, selectedLng], { draggable: true }).addTo(map)
        .bindPopup('موقع المحل - المحمدي للأقمشة').openPopup();
      marker.on('dragend', () => {
...
📦 كاش عند الاستلام`;
  ...
  if (!waNumber || waNumber.includes('X') || waNumber.includes('x')) {
    alert('يرجى ضبط رقم الواتساب في js/main.js داخل المتغير waNumber ليعمل الطلب.');
    return;
  }

  // تم الاعتماد على الرابط الثابت بموقع المحل
  const fixedMapLink = `https://www.google.com/maps?q=${homeLat},${homeLng}`;
